...
def company_dashboard(request):
    ...
    if request.user.is_superuser and not hasattr(request.user, 'employee'):
        # Obtener la compañía seleccionada o usar la primera disponible
        admin_company_id = request.session.get('admin_company_id')
        if admin_company_id:
            try:
                company = Company.objects.get(id=admin_company_id)
            except Company.DoesNotExist:
                company = Company.objects.first()
                if company:
                    request.session['admin_company_id'] = company.id
                else:
                    messages.error(request, "No hay compañías disponibles")
                    return redirect('select_admin_company')
        else:
            company = Company.objects.first()
            if company:
                request.session['admin_company_id'] = company.id
...
@login_required
@user_passes_test(lambda u: u.is_superuser)
def select_admin_company(request):
    if request.method == 'POST':
    ...
    current_company_id = request.session.get('admin_company_id')
    ...
    return render(request, 'dashboard/admin/select_company.html', {
        'companies': companies,
